...
class Actor(nn.Module):
    def __init__(self, state_dim, action_dim, max_action):
        ...
        self.layer3 = nn.Linear(256, action_dim)        # Output layer, outputs action_dim neurons
    ...
    def forward(self, state):
        ...
        x = self.max_action * torch.sigmoid(self.layer3(x)) # Apply sigmoid to bound action between 0 and 1, then scale by max_action
        # x = self.max_action * torch.relu(self.layer3(x))
...
class Critic(nn.Module):
    def __init__(self, state_dim, action_dim):
        ...
        self.layer2 = nn.Linear(256, 256)                       # Second hidden layer with 256 neurons
        self.layer3 = nn.Linear(256, 1)                         # Output layer, outputs a single Q-value
    ...
    def forward(self, state, action):
        ...
        x = torch.relu(self.layer2(x))          # Apply ReLU activation after the second layer
        x = self.layer3(x)                      # Output a single Q-value (no activation on the output)
